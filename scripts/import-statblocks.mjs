...
const statblockRoot = path.resolve(process.argv[2] ?? path.join(projectRoot, "..", "source_statblocks", "VTT_Statblocks"));
const indexPath = path.resolve(process.argv[3] ?? path.join(projectRoot, "..", "upload", "NPC_Index(1).csv"));
...
console.log(`Imported ${npcs.length} statblocks into ${path.relative(projectRoot, outputPath)}`);
